...
- **Theme**: permanent navy/gold palette (`--color-navy-*`/`--color-gold-*` tokens in `globals.css`, dark-only — no more OS light-mode toggle). New shared `Header.tsx` (site-wide nav + auth status, wired into `layout.tsx`) replaced the duplicated sign-in UI that used to live only on the home page.
- UI lives under `src/app/market-prices/` (`MarketPricesApp.tsx`, `ItemPicker.tsx`, `CategoryTree.tsx`, `PriceGrid.tsx`, `PriceHistoryChart.tsx`, `FavoritesTab.tsx`, `categoryTaxonomy.ts`, `types.ts`, `actions.ts`).
